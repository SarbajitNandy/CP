...
int minimumCostSimplePath(int u, int destination,bool visited[], int (&graph)[size_x][size_y])
{
...
void solve()
{
    ...
    int graph[1000][1000]={-1}; memset(graph, -1, sizeof graph);
    int m; cin >> m;
    int v1,v2;
    for(int i=0;i<m;i++) {
        cin >> v1 >> v2;
        cin >> graph[nodeMap[v1]][nodeMap[v2]];
    }
