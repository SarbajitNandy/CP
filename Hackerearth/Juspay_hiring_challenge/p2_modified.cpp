...
bool minimumCostSimplePath(int u, int destination,bool visited[], vector<pair<int,int>> *graph)
{
    ...
    for(auto each : graph[u]) {
        if (!visited[each.ff]) {
            if (minimumCostSimplePath(each.ff,destination, visited, graph)) return true;
        }
     }
