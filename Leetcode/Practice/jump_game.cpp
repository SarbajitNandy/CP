...
class Solution {
public:
    bool go(vector<int> &arr, int start, bool vis[]) {
        
        if (start>=arr.size() || start<0 || vis[start]) return false;
        if (arr[start]==0)return true;
        cout << start << " " << vis[start] << endl;
        vis[start]=true;
        // int lt=start-arr[start];
        // int rt = start+arr[start];
        return go(arr, start-arr[start], vis) || go(arr, start+arr[start], vis);
    }
    bool canReach(vector<int>& arr, int start) {
    	int n=arr.size();
        bool vis[n]; memset(vis,0,n);
        return go(arr,start,vis);
    }
};
...
{
	int n; cin >> n;
	vi arr(n,0); for (int i=0;i<n;i++) cin >> arr[i];
